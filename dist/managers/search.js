 ...
 */
import { calculateEngagementScore, parseDateTime, DEFAULT_SETTINGS, } from '../models/index.js';
/**
...
export class SearchManager {
    ...
    filterContent(items, filters) {
        ...
        if (patterns) {
            filtered = filtered.filter((item) => this.passesKeywordFilters(item.text, patterns));
        }
        ...
        if (filters.subjectContains) {
            ...
            filtered = filtered.filter((item) => {
                ...
                const subjectUri = item.rawData?.subject_uri;
        ...
        if (filters.subjectHandleContains) {
            ...
            filtered = filtered.filter((item) => {
                ...
                const subjectHandle = item.rawData?.subject_handle;
     ...
     */
    sortResults(items, mode) {
        const sorted = [...items];
        const getDate = (item) => parseDateTime(item.createdAt)?.getTime() ?? 0;
        const getEngagement = (item) => calculateEngagementScore(item.likeCount, item.repostCount, item.replyCount);
        switch (mode) {
            case 'newest':
    ...
    calculateStatistics(items) {
        ...
        const postsAndReplies = items.filter((item) => item.contentType === 'posts' || item.contentType === 'replies');
        // Calculate totals
        let totalLikes = 0;
        let totalReposts = 0;
        let totalReplies = 0;
        ...
        for (const item of postsAndReplies) {
            ...
            const score = calculateEngagementScore(item.likeCount, item.repostCount, item.replyCount);
     ...
     */
    compileSearchPatterns(keywords) {
        ...
        const negative = negativePatterns.map((p) => new RegExp(p.pattern, p.caseSensitive ? '' : 'i'));
        const required = requiredPatterns.map((p) => new RegExp(p.pattern, p.caseSensitive ? '' : 'i'));
        return { positive, negative, required };
    }
    /**
     * Check if text passes keyword filter criteria
     */
    passesKeywordFilters(text, patterns) {
        if (!text) {
            // If no text and we have positive/required patterns, fail
            return (!patterns.positive.caseSensitive &&
        ...
        const hasPositive = patterns.positive.caseSensitive || patterns.positive.caseInsensitive;
        if (hasPositive) {
            const matchesSensitive = patterns.positive.caseSensitive?.test(text) ?? false;
            const matchesInsensitive = patterns.positive.caseInsensitive?.test(text) ?? false;
