...
export class DeletionManager {
    ...
    async deleteFromRemote(uri) {
        ...
        try {
            ...
            throw new NetworkError(`Failed to delete from remote: ${error instanceof Error ? error.message : 'Unknown error'}`, error && typeof error === 'object' && 'status' in error
                ? error.status
                : undefined);
        }
    }
     ...
     * ```
     */
    async batchDelete(uris, onProgress) {
        const result = {
            success: 0,
            failed: 0,
            errors: [],
