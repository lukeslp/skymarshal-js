...
export class RelationshipManager {
     ...
     */
    async getNonFollowersCount(did) {
     ...
     */
    async getLowRatioFollowing(did, maxRatio = 0.1, minFollowing = 100) {
     ...
     */
    async bulkUnfollow(dids, options = {}) {
     ...
     * @returns Result of bulk operation
     */
    async bulkUnfollowByRule(did, rules, options = {}) {
        const { minRatio = 0.1, minFollowingForRatio = 100, delayMs = 500, stopOnError = false, } = options;
        // Get all following
        const following = await this.networkManager.getAllFollowing(did);
        // Collect DIDs to unfollow based on rules
        const toUnfollow = new Set();
     ...
     */
    hasLowRatio(profile, maxRatio, minFollowing) {
