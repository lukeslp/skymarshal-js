...
import type { DatabaseSchema, ObjectStoreDefinition } from './schema.js';
 ...
 */
function createObjectStore(db: IDBDatabase, storeDef: ObjectStoreDefinition): void {
...
export const migrations: DBMigration[] = [
  {
    ...
    migrate: (_db: IDBDatabase, _transaction: IDBTransaction) => {
...
export function runMigrations(
  ...
  for (const migration of applicableMigrations) {
    ...
    try {
      ...
      const transaction = db.transaction as unknown as IDBTransaction;
 ...
 */
export function getMigration(version: number): DBMigration | undefined {
...
export function validateMigrations(): void {
  ...
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i] - sorted[i - 1] > 1) {
      console.warn(
        `Migration version gap detected: v${sorted[i - 1]} -> v${sorted[i]}`
      );
    }
  }
}
