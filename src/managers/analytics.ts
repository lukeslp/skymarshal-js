...
/** Ratio pattern checker */
type RatioPattern = (followers: number, following: number, posts?: number) => boolean;

/** Ratio patterns for bot detection */
const RATIO_PATTERNS: Record<string, RatioPattern> = {
...
const RATIO_WEIGHTS: Record<string, number> = {
 ...
 */
export function getEngagementPreset(score: number, average: number): EngagementPreset {
  if (average === 0) {
  ...
  if (ratio < 0.1) return 'dead';      // <10% of average
  if (ratio < 0.5) return 'bombers';   // 10-50% of average
  if (ratio < 1.0) return 'mid';       // 50-100% of average
  if (ratio < 5.0) return 'bangers';   // 100-500% of average
...
export class AnalyticsManager {
  ...
  analyzeAccount(profile: Profile | { 
    ...
    const followers = (profile as any).followersCount ?? (profile as any).followers_count ?? 0;
    const following = (profile as any).followsCount ?? (profile as any).follows_count ?? 0;
    const posts = (profile as any).postsCount ?? (profile as any).posts_count ?? 0;
    ...
    let recommendation: AccountAnalysis['recommendation'];
    if (botScore >= 0.7) {
  ...
  categorizeByEngagement(posts: Array<{
    ...
    const categories: Record<EngagementPreset, string[]> = {
      dead: [],
      bombers: [],
      mid: [],
      bangers: [],
  ...
  getTopPosts(posts: Array<{
    ...
    repostCount?: number;
    replyCount?: number;
  }>, limit: number = 10): EngagementAnalysis[] {
    const analyses = this.analyzeEngagement(posts);
