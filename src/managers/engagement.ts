...
export class EngagementManager {
  ...
  private cache: Map<string, EngagementMetrics> = new Map();
  ...
  constructor(agent: AtpAgent, ttlConfig: TTLConfig = DEFAULT_TTL) {
    this.agent = agent;
  ...
  calculateTTL(createdAt: string): number {
    ...
    if (postAge < oneDay) return this.ttlConfig.recent;
    if (postAge < sevenDays) return this.ttlConfig.medium;
   ...
   */
  async batchUpdateEngagement(uris: string[]): Promise<Map<string, EngagementMetrics>> {
    ...
    try {
      const fetchedMetrics = await parallelMap(
        staleUris,
        async (uri) => this.fetchEngagementFromAPI(uri),
   ...
   */
  async hydrateItems(items: PostWithEngagement[]): Promise<void> {
   ...
   */
  private shouldUpdateEngagement(item: PostWithEngagement): boolean {
   ...
   */
  private async fetchEngagementFromAPI(uri: string): Promise<EngagementMetrics | null> {
    try {
      this.stats.apiCalls++;
      ...
      const record = post.record as { createdAt?: string } | undefined;
   ...
   * @returns Like count
   */
  async fetchLikeCount(uri: string): Promise<number> {
    try {
      const response = await fetch(
        `${this.config.baseUrl}/app.bsky.feed.getLikes?uri=${encodeURIComponent(uri)}&limit=${this.config.pageSize}`
      );
   ...
   */
  async fetchRepostCount(uri: string): Promise<number> {
    try {
      const response = await fetch(
        `${this.config.baseUrl}/app.bsky.feed.getRepostedBy?uri=${encodeURIComponent(uri)}&limit=${this.config.pageSize}`
      );
