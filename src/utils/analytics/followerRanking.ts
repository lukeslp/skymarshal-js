 ...
 * ```
 */
export function calculateRatio(followersCount: number, followsCount: number): number {
  if (followsCount === 0) {
 ...
 * ```
 */
export function calculateEngagementRate(postsCount: number, followersCount: number): number {
 ...
 * ```
 */
export function getInfluenceTier(followersCount: number): InfluenceTier {
  if (followersCount >= 100000) return 'mega';
  if (followersCount >= 10000) return 'macro';
  if (followersCount >= 1000) return 'micro';
  return 'nano';
}
 ...
 * ```
 */
export function calculateInfluenceScore(profile: Pick<RankableProfile, 'followersCount' | 'followsCount' | 'postsCount'>): number {
 ...
 * ```
 */
export function calculateInfluenceMetrics(profile: RankableProfile): InfluenceMetrics {
  const { followersCount, followsCount, postsCount } = profile;
 ...
 * ```
 */
export function rankByInfluence(profiles: RankableProfile[]): RankedProfile[] {
 ...
 * ```
 */
export function groupByTier(profiles: RankableProfile[]): ProfilesByTier {
  const ranked = rankByInfluence(profiles);

  const grouped: ProfilesByTier = {
    mega: [],
    macro: [],
    micro: [],
    nano: [],
 ...
 * ```
 */
export function getTopInfluencers(profiles: RankableProfile[], n: number): RankedProfile[] {
  const ranked = rankByInfluence(profiles);
 ...
 * ```
 */
export function filterByMinScore(profiles: RankableProfile[], minScore: number): RankedProfile[] {
  const ranked = rankByInfluence(profiles);
 ...
 * ```
 */
export function filterByTier(profiles: RankableProfile[], tiers: InfluenceTier | InfluenceTier[]): RankedProfile[] {
 ...
 * ```
 */
export function getInfluenceStats(profiles: RankableProfile[]): {
  ...
  // Count by tier
  const tierCounts: Record<InfluenceTier, number> = { mega: 0, macro: 0, micro: 0, nano: 0 };
  ranked.forEach((p) => {
...
export function compareInfluence(
  profileA: RankableProfile,
  profileB: RankableProfile
): {
  winner: 'a' | 'b' | 'tie';
  scoreDiff: number;
