...
export function isValidAtUri(uri: string): boolean {
  return /^at:\/\/did:[a-z0-9]+:[a-zA-Z0-9._%-]+\/app\.bsky\.(feed\.post|feed\.like|feed\.repost|actor\.profile|graph\.follow)\/[a-z0-9]+$/i.test(
    uri
  );
}
 ...
 * ```
 */
export function validatePostText(text: string): PostTextValidation {
  const errors: string[] = [];
...
export function validateImageFile(file: File): ValidationResult {
  ...
  if (file.size > MAX_SIZE) {
    errors.push(
      `File too large: ${Math.round(file.size / 1024)}KB. Maximum: 976KB`
    );
  }
...
export function isValidPostUrl(url: string): boolean {
  try {
    ...
    if (!urlObj.hostname.includes('bsky.app')) return false;
 ...
 * ```
 */
export function detectSpamPatterns(text: string): SpamDetectionResult {
