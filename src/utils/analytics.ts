 ...
 */
export function hasDefaultHandle(handle?: string): boolean {
 ...
 */
export function hasSuspiciousUrls(bio?: string): boolean {
  if (!bio) return false;
 ...
 */
export function hasRoundFollowingCount(following: number): boolean {
 ...
 */
export function getAccountAgeInDays(createdAt?: string): number | null {
 ...
 * ```
 */
export function analyzeBotSignals(profile: AccountProfile): BotAnalysisResult {
  ...
  let category: BotAnalysisResult['category'];
  if (totalScore >= 4) {
    category = 'bot_likely';
 ...
 */
export function batchAnalyzeBotSignals(accounts: AccountProfile[]): BotAnalysisResult[] {
 ...
 */
export function calculateBotAnalysisSummary(results: BotAnalysisResult[]): {
