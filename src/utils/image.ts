 ...
 * ```
 */
export async function createPlaceholder(file: File | Blob): Promise<string> {
  if (!isBrowserWithCanvas()) {
    return '';
  }
...
export async function convertHEIC(file: File): Promise<File> {
  if (typeof window === 'undefined' || typeof (window as unknown as { heic2any?: unknown }).heic2any !== 'function') {
    throw new Error('heic2any library not loaded. Install and import heic2any, then assign to window.heic2any');
  }

  const heic2any = (window as unknown as { heic2any: (options: { blob: Blob; toType: string; quality: number }) => Promise<Blob> }).heic2any;
 ...
 * ```
 */
export async function optimizeForBluesky(
  file: File | Blob,
  width: number,
  height: number,
  ...
  for (let i = 0; i < Math.min(qualityLevels.length, maxAttempts); i++) {
    ...
    if (optimized.size <= BLUESKY_MAX_SIZE) {
      console.log(
        `Optimized to ${Math.round(optimized.size / 1024)}KB with quality ${qualityLevels[i]}`
      );
      return optimized;
    }
  }
...
export async function processImage(file: File): Promise<ProcessedImage> {
  ...
  // Optimize if needed
  let optimized: Blob = processedFile;

  if (processedFile.size > BLUESKY_MAX_SIZE) {
    console.log(
      `Image ${Math.round(processedFile.size / 1024)}KB exceeds 976KB limit, optimizing...`
    );
    try {
      optimized = await optimizeForBluesky(
        processedFile,
        dimensions.width,
        dimensions.height
      );
